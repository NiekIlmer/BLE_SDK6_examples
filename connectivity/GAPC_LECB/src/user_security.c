...
{
	arch_printf(" \r\n Start adv %x", ecdh_key.public_key.x[0]);
	smpm_ecdh_key_create_func(NULL);
...
uint8_t pairing_conidx;
const struct gapc_bond_req_ind *pairing_param;

void pairing_request_callback(){
}

void user_app_on_pairing_request(uint8_t conidx, struct gapc_bond_req_ind const *param){
	pairing_conidx = conidx;
	pairing_param = param;
  //app_easy_timer(100, pairing_request_callback);
	default_app_on_pairing_request(pairing_conidx, pairing_param);
}
